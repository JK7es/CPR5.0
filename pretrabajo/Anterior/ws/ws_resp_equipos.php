...
//echo $nombre;
	...
	$consulta 	= "SELECT DISTINCT id_equipo, equipo, eq.id_categoria 
				   FROM equipos eq, categorias cat
				   WHERE eq.id_categoria = cat.id_categoria
				   AND temporada =  YEAR(CURDATE()) ";
	$consulta 	= $consulta . "AND UPPER(equipo) LIKE '%" . $nombre . "%' ";
	$consulta 	= $consulta . "AND cat.sexo = '" . $sexo . "' ";
	$consulta 	= $consulta . "ORDER BY id_categoria, equipo";					
